...
{
    /// <summary>
    /// 基于树的并查集实现 Quick Union的优化,基于rank优化,使用路径压缩_parent[p] = _parent[_parent[p]],将之前的树高度减少
    /// 合并两个集合,即使一个集合的根节点指向另一个集合的根节点
    /// </summary>
    public class UnionCheckingSet5
    {
        /// <summary>
        /// 并查集数组
        /// </summary>
        ...
        public UnionCheckingSet5(int size)
        {
        ...
        private int Find(int p)
        {
            ...
            while (p != _parent[p])
            {
                _parent[p] = _parent[_parent[p]];//only add this sentence
                p = _parent[p];
            }
